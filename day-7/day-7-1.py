...
class Directory:
    ...
    def get_better_directories_to_delete(self):
        children_size = 0

        if self.size < SIZE_LIMIT:
            children_size = self.size

        for directory in self.directories:
            children_size += self.directories[directory].get_better_directories_to_delete()
...
print(root.get_better_directories_to_delete())
